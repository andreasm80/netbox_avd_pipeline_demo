...
def main(args):
    ...
    # Create a JSON request to filter out historical events and specific device IDs
    current_time = Timestamp()
    current_time.GetCurrentTime()  # Sets to current UTC time
    json_request = {
        "time": {
            "start": current_time.ToJsonString()
        },
        "filter": {
            "deviceIds": {
                "values": device_ids
    ...
    with grpc.secure_channel(args.server, connCreds) as channel:
        ...
        try:
            print("Attempting to subscribe to live Change Control events for specified device IDs...")
            # Iterate directly over the stream to process events as they arrive
